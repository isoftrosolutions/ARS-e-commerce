        ...
        <div class="flex flex-col lg:flex-row gap-12">
            ...
            <div class="flex-grow space-y-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" rows="2"
                                      class="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-brand-500/20 outline-none transition-all" placeholder="Apartment, suite, or unit number etc."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 md:p-10 rounded-[2.5rem] soft-shadow border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <!-- COD -->
                        <label class="relative flex flex-col p-6 bg-slate-50 rounded-3xl border-2 border-transparent hover:border-brand-500/20 cursor-pointer transition-all peer-checked:bg-white has-[:checked]:bg-white has-[:checked]:border-brand-600 has-[:checked]:shadow-xl has-[:checked]:shadow-brand-600/5 group">
                    ...
                    <div id="payment-proof-section" class="hidden mt-10 pt-10 border-t border-slate-100 animate-in fade-in slide-in-from-top-4 duration-300">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Upload Screenshot</label>
                                    <input type="file" name="payment_proof" id="p_proof" accept="image/*"
            ...
            <div class="w-full lg:w-96 flex-shrink-0">
                <div class="bg-white rounded-[2.5rem] p-8 md:p-10 soft-shadow border border-slate-100 sticky top-28">
                    ...
                    <div class="space-y-4 mb-8 max-height-[300px] overflow-y-auto no-scrollbar">
                        <?php foreach($cart as $id => $item): ?>
                        <div class="flex gap-4">
                            <div class="w-14 h-14 bg-slate-50 rounded-xl border border-slate-100 p-2 flex-shrink-0">
                                <img src="<?= $item['image'] ? UPLOAD_DIR . $item['image'] : 'https://via.placeholder.com/100' ?>" class="w-full h-full object-contain">
                            </div>
                            <div class="flex-grow">
                                <p class="text-xs font-bold text-slate-900 line-clamp-1"><?= htmlspecialchars($item['name']) ?></p>
                                <p class="text-[10px] font-bold text-slate-400 uppercase mt-1"><?= $item['qty'] ?> x <?= formatPrice($item['price']) ?></p>
                            </div>
